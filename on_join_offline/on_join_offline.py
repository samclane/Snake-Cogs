...
class OnJoin:
    """Uses espeak to announce when a user joins the channel, like Teamspeak or Ventrillo"""

    def __init__(self, bot):
        self.bot = bot
        self.audio_players = {}
        self.settings = dataIO.load_json("data/on_join_offline/settings.json")

        self.save_path = "data/on_join_offline/"
        ...
        dataIO.save_json("data/on_join_offline/settings.json", self.settings)
    ...
    def string_to_speech(self, text):
        ...
        with open(self.save_path + "temp_text", "w+") as file:
            file.write(text)
        call(['espeak -v{}+{} -s{} -f {} --stdout > {}'.format(self.settings["locale"], self.settings["voice"],
                                                               self.settings["speed"], self.save_path + "temp_text",
...
def check_folders():
    if not os.path.exists("data/on_join_offline"):
        print("Creating data/on_join_offline folder...")
        os.makedirs("data/on_join_offline")


def check_files():
    f = "data/on_join_offline/settings.json"
    if not dataIO.is_valid_json(f):
        print("Creating default on_join_offline settings.json...")
        dataIO.save_json(f, {})
